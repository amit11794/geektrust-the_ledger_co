...
namespace geektrust_the_ledger_co
{
    class Commands
    {
    ...
    class the_ledger_co
    {
        public static void Main(string[] args)
        {
            ...
            string file = Path.Combine(path, "input1.txt"); // copy the file from "input" folder to bin
            ...
            foreach (string line in lines)
            {
                ...
                switch (lstInput[0])
                {
                    ...
                    case Commands.BALANCE:
                        long TOTAL_AMOUNT_PAID = 0;
                        int NO_OF_EMIS_LEFT = 0;
                        GetBalanceDetails(lstInput, lstLoan, lstPayment, ref TOTAL_AMOUNT_PAID, ref NO_OF_EMIS_LEFT);
                        Console.Out.WriteLine(string.Format("{0} {1} {2} {3}", lstInput[1], lstInput[2], TOTAL_AMOUNT_PAID, NO_OF_EMIS_LEFT));
                        break;
        ...
        private static void GetBalanceDetails(List<string> lstInput, List<Loan> lstLoan, List<Payment> lstPayment, ref long TOTAL_AMOUNT_PAID, ref int NO_OF_EMIS_LEFT)
        {
            ...
            TOTAL_AMOUNT_PAID = LUMP_SUM_AMOUNT_PAID + (AMOUNT_PER_EMI * EMI_NO);
            ...
            NO_OF_EMIS_LEFT = Convert.ToInt32(Math.Ceiling(AMOUNT_DUE / AMOUNT_PER_EMI));
        }
    }
    class Loan
    {
